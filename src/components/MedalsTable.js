...
const MedalsTable = ({ scores }) => {
  ...
  const [rankings, setRankings] = useState(() =>
    calculateRanking(contestantScores)
  )

  return (
    <tbody id="medal-totals">
      <tr>
        {scores.map(winner => (
          <td key={winner.name} className="medals">
            <ContestantMedals
              scores={contestantScores[winner.name]}
              place="first"
            />
            <ContestantMedals
              scores={contestantScores[winner.name]}
      ...
      <tr>
        {scores.map(winner => (
          <td key={winner.name}>
            <span className="mx-1">
              {contestantScores[winner.name].third}x{" "}
              <FontAwesomeIcon icon="medal" className="third" />
            </span>
            <span className="mx-1">
              {contestantScores[winner.name].second}x{" "}
              <FontAwesomeIcon icon="medal" className="second" />
            </span>
            <span className="mx-1">
              {contestantScores[winner.name].first}x{" "}
              <FontAwesomeIcon icon="medal" className="first" />
            </span>
          </td>
        ))}
      </tr>
      <tr>
        {scores.map(winner => (
          <td key={winner.name}>
            <FontAwesomeIcon
              icon="medal"
              className={`mr-2 ${rankings[winner.name]}`}
            />
